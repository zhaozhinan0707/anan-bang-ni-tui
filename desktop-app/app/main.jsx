  ...
  const sendAssistant = async () => {
    ...
    try {
      if (message.replace(/\s/g, '').includes('以图搜图')) {
        ...
        saveChat([...pending, { id: uid(), role: 'assistant', text: '已复制当前图片并打开 Pinterest 首页。点击搜索框右侧的 Lens 相机，选择上传图片后可直接粘贴（Ctrl+V）当前原图开始搜索；左键打开图片详情，右键选择“发送图片到 Prompt Vault 画布”即可把图片和来源网址送回当前画布。', source: 'image-search', createdAt: Date.now() }])
        return
      }
