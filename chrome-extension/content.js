 ...
 *  此时 src/currentSrc 还是占位图，所以必须先查懒加载属性 + srcset 选最大尺寸 */
  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    if (msg && msg.type === "PV_GET_IMAGE_SOURCE_URL") {
      const target = String(msg.imageUrl || "");
      const image = [...document.images].find((item) => item.currentSrc === target || item.src === target || item.getAttribute("src") === target);
      const anchor = image && (image.closest("a[href]") || image.parentElement?.closest("a[href]"));
      sendResponse({ url: anchor ? anchor.href : location.href });
      return;
    }
  ...
  function scanPageImages() {
    ...
    for (const img of imgs) {
      ...
      out.push({
        ...
        prompt: nearby ? nearby.prompt : "",
        alt: (img.getAttribute("alt") || img.getAttribute("title") || "").trim(),
        sourceTool: nearby ? (nearby.sourceTool || detectSource()) : detectSource()
      });
    }
